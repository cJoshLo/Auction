...
def finished(request, Items_id):
    items_full = Items.objects.get(pk=Items_id)
    buyersName = items_full.currentBider.username
    finalPrice = items_full.startingBid
    itemName = items_full.title
    itemDesciption = items_full.description
    itemImage = items_full.image.url
    items_full.delete()
